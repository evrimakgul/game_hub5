...
begin
  ...
  from public.character_known_powers as kp
  where kp.character_id = target_character_id
    and kp.power_id = target_power_id;
  ...
  set current_mana = c.current_mana - mana_cost
  where c.character_id = target_character_id
  returning *
  ...
  from public.character_status_effects as cse
  ...
  if existing_status_effect_id is null then
    ...
    returning public.character_status_effects.character_status_effect_id
    into existing_status_effect_id;
  else
    update public.character_status_effects as cse
    set
    ...
    where cse.character_status_effect_id = existing_status_effect_id;
  end if;
