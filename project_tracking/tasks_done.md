...
- `CS-UI-REFAC-01` Rebuilt `/player/character` into the fixed dashboard layout with top read-only overview, middle compact summaries, mid-section loadout popup pickers, a single bottom Inventory workspace for loadout plus inventory items, real left/right earring loadout slots, and inventory-active charm handling while preserving existing sheet mechanics.
- `LIVE-PLAYER-SYNC-01` Added player-owner metadata and Supabase campaign/session snapshot hydration so DM rewards sync back into the owning player's character sheet while player flow hides other players' characters.
- `ACCOUNT-PLAYER-CHAR-01` Added Supabase account-level player character rows, `/player` upload for legacy local-only characters, server-backed player-hub hydration, and localStorage filtering for server-backed player characters.
