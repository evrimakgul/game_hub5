...
- `ITEM-REFAC-01` Refactor the current fixed item category/subtype assumptions into persisted DM-defined category and subcategory definitions. The target data model should support `name`, `parent category`, `allowedEquipSlots[]`, `occupiedSlots[]`, and constrained `mechanicalRole` values such as `melee`, `range`, `shield`, `body_armor`, `occult`, `consumable`, and `accessory`. This is not a single class change; it must replace fixed-category assumptions safely across blueprint creation, blueprint persistence, item-instance hydration, equip validation, loadout rendering, combat attack selection, summary labels, legacy migration, and test coverage. Tackle this in a dedicated refactor thread before more item-system expansion work.
